module AccessesHelper
  ...
  def board_watchers_list(board)
    ...
    tag.div(class: "divider divider--fade") do
      tag.strong(watchers.any? ? I18n.t("boards.involvement.watching_for_new_cards") : I18n.t("boards.involvement.no_one_watching"), class: "txt-uppercase")
    end +
    tag.div(avatar_tags(displayed_watchers), class: "board-tools__watching") do
      tag.div(data: { controller: "dialog", action: "keydown.esc->dialog#close click@document->dialog#closeOnClickOutside" }) do
        tag.button("+#{overflow_count}", class: "overflow-count btn btn--circle borderless", data: { action: "dialog#open" }, aria: { label: I18n.t("boards.involvement.show_more_watchers", count: overflow_count) }) +
  ...
  def involvement_button(board, access, show_watchers, icon_only)
    label_text = access.access_only? ? I18n.t("boards.involvement.watch_this") : I18n.t("boards.involvement.stop_watching")
    button_to(
